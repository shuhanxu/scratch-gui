...
class SBFileUploader extends React.Component {
    ...
    componentDidUpdate (prevProps) {
        ...
        //     // this.reader.readAsArrayBuffer(this.loadBlob);
            
        // }
        console.log('componentDidUpdate')
        console.log(this.reader)
        this.reader.readAsArrayBuffer(window.lastContent);
    ...
    getProjectTitleFromFilename (fileInputFilename) {
        console.log('getProjectTitleFromFilename')
    ...
    handleChange (e) {
        ...
        if (thisFileInput.files) { // Don't attempt to load if no file was selected
            ...
            // we must console with the user that they really intend to replace it.
            // (If they don't own the project and haven't changed it, no need to console.)
            let uploadAllowed = true;
            if (userOwnsProject || (projectChanged && isShowingWithoutId)) {
                // uploadAllowed = console( // eslint-disable-line no-alert
    ...
    onload () {
        ...
        // this.reader.readAsArrayBuffer(window.lastContent);
        console.log(this.reader.result)
        if (this.reader && this.loadBlob && this.reader.result) {
            this.props.onLoadingStarted();
            const filename = '123123' ||(this.fileToUpload && this.fileToUpload.name);
            console.log(this.reader.result)
            this.props.vm.loadProject(this.reader.result)//this.reader.result)
                .then(() => {
    ...
    handleClick () {
        ...
        //  this.onload()
        var self = this
        window.parent.postMessage({messageType: 'load'}, '*');
        this.bindEvent(window, 'message', function (e) {
            if(e.data.messageType === 'load'){
                console.log('scratch load')
                console.log(e)
                self.loadBlob = e.data.data
                console.log(111223)
                console.log(self.loadBlob)
                // self.reader = new FileReader();
                self.resetFileInput();
                self.componentDidUpdate()
                // self.reader.onload = this.onload;
                // self.resetFileInput();
                // self.onload()
            }
        });
